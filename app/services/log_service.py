...
class LogService:
    ...
    def create(self, log_data: HabitLogCreate):
        ...
        response = self.db.create(self.table_name, log_data.model_dump(mode='json'))
        return response.data[0] if response.data else None
    ...
    def get_logs(self, user_id: UUID, habit_id: Optional[UUID] = None, day: Optional[date] = None):
        ...
        query = self.db.client.table(self.table_name).select("*, habits!inner(user_id)")
    ...
    def get_logs_by_timeframe(self, user_id: UUID, start_date: date, end_date: date, habit_ids: Optional[List[UUID]] = None):
        ...
        habit_query = self.db.client.table("habits").select("*").eq("user_id", str(user_id)).eq("is_active", True)
        if habit_ids:
        ...
        log_query = self.db.client.table(self.table_name).select("*")
    ...
    def delete(self, log_id: UUID):
        # Ownership check is done in the router
        response = self.db.delete(self.table_name, filters={"id": str(log_id)})
        return len(response.data) > 0 if response.data else False

    def get_by_id(self, log_id: UUID):
        """Fetch a log with its habit info to check ownership."""
        response = self.db.client.table(self.table_name)\
